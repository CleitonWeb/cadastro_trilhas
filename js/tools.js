...
function validarTermos() {
  ...
  if (!termosCheckbox.checked) {
    erroTermos.textContent = 'Você precisa aceitar o termo de uso!';
...
function validarSenhas() {
  ...
  const erroSenha = document.getElementById('error_senha');
  const erroConfirmaSenha = document.getElementById('error_confirma_senha');
  ...
  if (senha.length < 8 || senha.length > 14) {
    erroSenha.textContent = 'A senha deve ter entre 8 e 14 caracteres.';
    senhaValida = false;
  }

  // 2. Validar combinação de letras, números e caracteres especiais
  const regexCombinacao = /^(?=.*[a-zA-Z])(?=.*\d)(?=.*[!@#$%^&*()_+{}\[\]:;<>,.?~\\/-]).+$/;
  if (!regexCombinacao.test(senha)) {
    erroSenha.textContent += (erroSenha.textContent ? '<br>' : '') + 'A senha deve conter letras, números e caracteres especiais.';
    senhaValida = false;
  }

  // 3. Validar pelo menos uma letra maiúscula
  const regexMaiuscula = /[A-Z]/;
  if (!regexMaiuscula.test(senha)) {
    erroSenha.textContent += (erroSenha.textContent ? '<br>' : '') + 'A senha deve conter pelo menos uma letra maiúscula.';
    senhaValida = false;
  }

  // 4. Validar não aceitar acentuação gráfica
  const regexAcento = /[áàâãéèêíìóòôõúùüç"'\´\`\~\^\¨]/i;
  if (regexAcento.test(senha)) {
    erroSenha.textContent += (erroSenha.textContent ? '<br>' : '') + 'A senha não pode conter acentuação gráfica.';
    senhaValida = false;
  }

  // 5. Validar não aceitar sequência numérica
  const regexSequenciaNumerica = /(\d)\1{2,}/; // Verifica se um dígito se repete 3 ou mais vezes seguidas
  if (regexSequenciaNumerica.test(senha)) {
    erroSenha.textContent += (erroSenha.textContent ? '<br>' : '') + 'A senha não pode conter sequências numéricas repetidas.';
    senhaValida = false;
  }
  ...
  if (senhaValida) {
  //  alert('Senhas válidas e correspondem!');
